$('document').ready(() => {
    ...
    $.ajax({
        type: "GET",
        url: "https://ihc-ehf.certificatec.com/backend/buscar.php",
        success: function(res) {
            let grafica = JSON.parse(res);
            potenciometro(grafica.bar);
        }
    });
    $('#tabla').DataTable({
        "ajax": {
...
potencia.addEventListener('input', (e) => {
    ...
    if (ax != potencia.value) {
        ...
        $.ajax({
            type: "GET",
            url: "https://ihc-ehf.certificatec.com/backend/setStatusSlider.php",
            data: dataString,
            success: function(res) {
                console.log(res);
                v.textContent = res;
    ...
    function drawChart() {
        ...
        */
        ...
        var options = {
            chart: {
                title: 'Examen IHC',
                subtitle: 'Eduardo, Erika y abdiel',
            }
        };
